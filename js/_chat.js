...
_copy.on('success', function(e) {
    layer.msg('Copy Successful',{time:600});
    e.clearSelection();
});
_copy.on('error', function(e) {
    layer.msg('Copy Successful',{time:1000}); //May have to be "Copy unSuccessful"
});
...
$(document).on('click','#edit_data', function(e) {
    ...
    layer.open({
        ...
        type: 1,
        title: 'Edit Question',
        btn: ['Submit','Cancel'], //May have to be lowercase
        content: `
...
const BOT_NAME = "O-GPT";
const PERSON_NAME = "Omni";
// Function to delete chat history records for a user ID using the API
function deleteChatHistory(userId) {
    layer.confirm('This will clear the chat history, are you sure? ', {
        btn: ['Confirm','Cancel'],  //Not sure about here
...
function sendMsg(msg) {
    ...
    .then(data => {
        ...
        eventSource.onerror = function (e) {
            msgerSendBtn.disabled = false;
            console.log(e);
            div.innerHTML += 'The maximum context limit has been reached, please start a new session!';
            eventSource.close();
        };
    })
    .catch(error => console.error(error)); 
...
function randomString(len) {
    len = len || 32;
    var $chars = 'ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz';    /****The confusing characters Are Removed oOLl,9gq,Vv,Uu,I1****/
